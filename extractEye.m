...
eyeDetector = vision.CascadeObjectDetector('EyePairBig');    %create detector object
...
for j=3:length(folds)               %images klasörü altındaki her bir dosyayı gezen for loop
    ...
    for i=1 : length(list)          %until each image is converted gray
        ...
        bboxes = step(eyeDetector,img);         %using detector object, find bounding boxes of detected faces
        ...
        for h=1:m
            ...
            facePosition2 = facePosition + [-90 -140 120 190]; 
            croppedFace = imcrop(img, facePosition2);
            
            %figure, imshow(croppedFace);
            imwrite(croppedFace ,(pathToSaved+fold+ "/"+list(i).name));
        end
        %imwrite(eyes ,(pathToSaved+fold+ "/"+list(i).name));         %save cropped=face to the new location
